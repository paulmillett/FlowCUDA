...
{		
	...
	for (int k=0; k<NZ; k+=kskip) {
		for (int j=0; j<NY; j+=jskip) {
			for (int i=0; i<NX; i+=iskip) {
				int ndx = k*NX*NY + j*NX + i;
				outfile << fixed << setprecision(prec) << 1000.0*u[ndx] << " "
					                                   << 1000.0*v[ndx] << " " 
													   << 1000.0*w[ndx] << endl;
			}
		}
...
void write_vtk_immersed_boundary_3D_rods(std::string tagname, int tagnum, int nBeads, int nBeadsPerRod, int nRods, beadrod* beads, rod* rods)
{
	...
	*/	
	...
	// -----------------------------------
	//	Write the orientation angle verses x-axis for each bead:
	// -----------------------------------
	...
	outfile << "LOOKUP_TABLE default" << endl;
	for (int i=0; i<nBeads; i++) {
		float theta = acos(abs(rods[beads[i].rodID].p.x));		
		outfile << theta << endl;
	}
	*/
	...
	*/
	...
	// -----------------------------------------------
	//	Write the orientation angle verses x-axis for each line:
	// -----------------------------------------------
	...
	// -----------------------------------------------
	//	Write the rod type for each line:
	// -----------------------------------------------
