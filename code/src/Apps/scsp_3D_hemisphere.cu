...
# include "scsp_3D_hemisphere.cuh"
...
scsp_3D_hemisphere::scsp_3D_hemisphere()
{		
...
void scsp_3D_hemisphere::initSystem()
{
	...
	cudaMemcpy(zIBref_start, zIBH_start, sizeof(float)*nNodes, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(zIBref_end, zIBH_end, sizeof(float)*nNodes, cudaMemcpyHostToDevice);
...
void scsp_3D_hemisphere::cycleForward(int stepsPerCycle, int currentCycle)
{
	...
	for (int step=0; step<stepsPerCycle; step++) {
		...
		update_node_ref_position_IBM3D
		<<<nBlocksIB,nThreads>>> 
		(xIBref,yIBref,zIBref,xIBref_start,yIBref_start,zIBref_start,
		xIBref_end,yIBref_end,zIBref_end,cummulativeSteps,nSteps,nNodes);
...
void scsp_3D_hemisphere::writeOutput(std::string tagname, int step)
{
