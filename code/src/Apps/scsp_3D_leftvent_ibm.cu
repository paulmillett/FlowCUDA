...
# include "scsp_3D_leftvent_ibm.cuh"
# include "../IO/GetPot"
# include <string>
# include <math.h>
using namespace std;  
...
scsp_3D_leftvent_ibm::scsp_3D_leftvent_ibm() : lbm(),ibm()
{		
...
scsp_3D_leftvent_ibm::~scsp_3D_leftvent_ibm()
{
	lbm.deallocate();
	ibm.deallocate();	
}
...
void scsp_3D_leftvent_ibm::initSystem()
{
	...
	for (int k=0; k<Nz; k++) {
		for (int i=0; i<Nx; i++) {
			...
			float rz = float(k - 50);
			float rx = float(i - 107);
			float rr = sqrt(rz*rz + rx*rx);
			if (rr < 15.0) lbm.setVoxelType(ndx,1);
		}	
	}
	...
	for (int k=0; k<Nz; k++) {
		for (int i=0; i<Nx; i++) {
			...
			float rx = float(i - 23);
			float rr = sqrt(rz*rz + rx*rx);
			if (rr < 15.0) lbm.setVoxelType(ndx,2);
		}	
	}
	...
	ibm.read_ibm_start_positions("input_LV_trimesh.txt");
	ibm.read_ibm_end_positions("input_LV_trimesh.txt");
	
	ibm.shift_start_positions(5.0,2.0,3.0);
	ibm.shift_end_positions(5.0,2.0,3.0);
...
void scsp_3D_leftvent_ibm::cycleForward(int stepsPerCycle, int currentCycle)
{
	...
	GetPot inputParams("input.dat");
	float velBC1 = inputParams("Iolet1/vBC",0.0);
	float velBC2 = inputParams("Iolet2/vBC",0.0);
	...
	for (int step=0; step<stepsPerCycle; step++) {
		...
		lbm.setIoletV(0,invel*velBC1);
		lbm.setIoletV(1,invel*velBC2);
...
void scsp_3D_leftvent_ibm::writeOutput(std::string tagname, int step)
{	
	// ----------------------------------------------
